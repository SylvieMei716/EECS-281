...
String& String::erase(size_t pos, size_t len) {
    ...
    // TODO: Delete the following line, just here so it compiles after being downloaded DONE

    // TODO: Implement erase() below. DONE
    len = std::min(len, sz - pos);
    for (size_t i = pos; i < sz - len; ++i) {
        cstr[i] = cstr[i + len];
    }
    sz -= len;
    cstr[sz] = '\0';
...
String& String::insert(size_t pos, const String& str) {
    ...
    for (size_t i = sz + len - 1; i > pos + len - 1; i--) {
        cstr[i] = cstr[i - len];
    }
...
size_t String::find_first_of(const String& str, size_t pos) {
    // TODO: Implement find_first_of() below. DONE

    // TODO: Delete the following 3 lines, just here so it compiles after being downloaded DONE
