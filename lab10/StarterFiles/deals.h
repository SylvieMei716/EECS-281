...
cost best_price(const std::vector<cost>& prices) {
    ...
    for (size_t i = 0; i < memo.size(); ++i) {
        memo[i] = std::vector<cost>(prices.size(), std::numeric_limits<cost>::max() / 2);
    }
    ...
    for (size_t j = 1; j < prices.size(); ++j) {
        for (size_t i = 0; i < memo.size(); ++i) {
            ...
            if (i == 0) {
                ...
                memo[i][j] = std::min(memo[5][j - 1], memo[i][j - 1] + discounted(prices[j]));
            }
            else {
                //Compare applying stamp to current row or to previous row
                memo[i][j] = std::min(memo[i][j - 1] + discounted(prices[j]), memo[i - 1][j - 1] + prices[j]);
            }
